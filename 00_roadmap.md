...
目标：不再徒手作业；能烧煮、切割、捆扎。依赖链：**石器 → 骨木器 → 纤维绳索 → 陶器 →（鞣革）**。
...
目标：稳定热量盈余，脱离每日觅食。依赖链：**选址 → 驯化种植 → 储粮 → 畜养 → 盐**。
...
目标：摆脱石头与木头。依赖链：**木炭窑 → 陶窑升级 → 石灰砂浆 → 铜冶炼 → 铁冶炼与锻造 → 玻璃 → 肥皂**。
...
目标：人力畜力之外的动力。依赖链：**水轮/风车 → 齿轮传动 → 简易机床 → 蒸汽**。

| 节点 | 关键知识 | 文章 |
| --- | --- | --- |
| 水力 | 水轮类型与功率估算 | ✓ 08_waterwheel.md |
| 风力 | 风车类型与对风 | ✓ 09_windmill_basics.md |
...
目标：知识不再依赖单个人脑；协作规模突破邓巴数。依赖链：**文字 → 介质（纸墨）→ 印刷 → 教育 → 制度（历法/度量衡/法律/货币）→ 电力通信**。
...
1. 每个节点先读"从零实现路径"，确认依赖链的前置文章都已具备。
2. 应急视角文章（现代物资可用时）与从零重建文章互为补充，通过文内链接跳转。
3. 高风险工艺（冶金、窑炉、蒸馏）先读"常见错误与限制"；节点 ✓ 仅指文章已登记，不是可实施性或安全认证。2026-09-17 安全复审明确：打井的支护、可靠空气检测及救援链尚未闭合，不具备条件时不下井，采用地表或雨水路线；未知矿洞和地下空间同样不能以明火判安全。
