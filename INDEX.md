...
知识按两条轴组织：
- **阶段轴**（0 即时生存 → 1 工具容器 → 2 定居农业 → 3 材料革命 → 4 动力机械 → 5 信息组织）：见 [00_roadmap.md](00_roadmap.md)——技术树、依赖链、文章登记表，**新读者从这里开始**。
- **主题轴**（12 个领域目录）：如下表。
...
| 11 基础科学 | [README](11_science/README.md) | 天文 → 几何测量 → 科学方法 |
| 12 文字与知识传承 | [README](12_knowledge/README.md) | 文字 → 造纸 → 印刷 → 教育 → 记录保存 |

## 阅读顺序建议

1. 先读 [00_roadmap.md](00_roadmap.md) 确定自己所处的重建阶段。
2. 阶段 0（一无所有）：火（01）→ 水（02）→ 避难（04）→ 食物（03）→ 伤照护（05）。
3. 每个节点先读文章的"从零实现路径"，确认依赖链前置文章已具备。
